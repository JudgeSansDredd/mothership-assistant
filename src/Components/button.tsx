...
export default function Button(props: React.PropsWithChildren<PropType>) {
  ...
  return (
    <button
      className={`${colorClass} border-2 border-black dark:border-white focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 focus:outline-none dark:focus:ring-blue-800 disabled:opacity-50 ${props.className}`}
      {...buttonProps}
    >
      {children}
    </button>
